...
function CouchesGeo({ element, canEdit }) {
  ...
  const load = async () => {
    const res = await api.get(`/couches-geo/${element.id}`)
    setCouches(res.data)
  }
  ...
  const handleAdd = async (e) => {
    ...
    await api.post('/couches-geo/', payload)
    setShowForm(false)
  ...
  const handleDelete = async (id) => {
    await api.delete(`/couches-geo/${id}`)
    load()
  }
...
function LiaisonsTab({ element, elements, canEdit }) {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await api.get(`/liaisons/chaine/${element.id}`)
      setChaine(res.data)
  ...
  const handleCreate = async (e) => {
    ...
    await api.post('/liaisons/', payload)
    setShowForm(false)
    load()
  }
...
function RecepageTab({ selected, mesures, canEdit, showMesureForm, setShowMesureForm,
  ...
  useEffect(() => {
    api.get(`/analytics/recepage/${selected.id}`)
